...
class QuestionnaireService {
  ...
  async getQuestionsForAssessment(assessmentId, userId) {
    ...
    mongoQuestions.forEach((q, idx) => {
      // In full/comprehensive mode, we don't deduplicate by control_ref 
      // to ensure the user sees every single question in the framework.
      if (!isFullMode) {
        ...
        // Just deduplicate by question_id to be safe
        if (seenQuestionIds.has(q.question_id)) return;
